...
Cost Insights has only two required configuration fields: a map of cloud `products` for showing cost breakdowns and `engineerCost` - the average yearly cost of an engineer including benefits. Products must be defined as keys on the `products` field. A user-friendly name and a Material UI icon are **required**. See [Material UI](https://material-ui.com/components/material-icons/) for supported icons.
