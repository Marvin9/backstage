...
async function yamlResolver(params: ResolverParams): Promise<JsonValue> {
  ...
  try {
    ...
    throw new Error(
      `Placeholder \$${params.key} failed to parse YAML or JSON data at ${params.value}, ${e}`,
    );
  }
...
async function textResolver(params: ResolverParams): Promise<JsonValue> {
...
function relativeLocation({
  ...
  if (typeof value !== 'string') {
    throw new Error(
      `Placeholder \$${key} expected a string value, in the form of an absolute URL or a relative path`,
    );
  }
